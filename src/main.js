...
class FlowFieldEffect {
    ...
    #drawLine(angle, x, y) {
        ...
        this.#ctx.beginPath();
        this.#ctx.moveTo(x, y);
        this.#ctx.lineTo(x + Math.cos(angle) * length, y + Math.sin(angle) * length); //length of lines
        this.#ctx.stroke();
    }
    animate(timeStamp) {
        ...
        if (this.timer > this.interval) {
            ...
            for (let y = 0; y < this.#height; y += this.cellSize) {
                for (let x = 0; x < this.#width; x += this.cellSize) {
                    const angle = (Math.cos(mouse.x * x * 0.00001) + Math.sin(mouse.y * y * 0.00001)) * this.radius; // zoom
